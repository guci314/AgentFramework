#!/usr/bin/env python3
"""
使用Gemini 2.5 Flash的销售数据分析演示 - 禁用超我
测试禁用超我在快速模型上的表现
"""
...
def main():
    """主函数"""
    print("=== 销售数据分析演示（Gemini 2.5 Flash + 禁用超我） ===")
    ...
    # 创建认知代理实例 - 使用Gemini，禁用超我
    cognitive_agent = CognitiveAgent(
        llm=llm_gemini,
        max_cycles=5,
        verbose=True,
        enable_super_ego=False  # 禁用超我
    )
    ...
    # 执行任务
    print(f"\n⚡ 开始执行销售数据分析任务（Gemini 2.5 Flash + 禁用超我）...")
    ...
    print(f"\n🎊 Gemini 2.5 Flash销售数据分析演示完成（禁用超我）！")
    print(f"🔧 测试目的: 验证在快速模型上禁用超我的表现")
