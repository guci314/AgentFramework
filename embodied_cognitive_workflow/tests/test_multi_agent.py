...
简单的测试脚本，验证Ego能否正确选择合适的Agent执行任务。
"""
...
def main():
    ...
    print("\n测试2：文件操作任务")
    print("-" * 40)
    result = cognitive_agent.execute_sync("创建一个test.txt文件，写入'Hello from multi-agent system!'")
    print(f"结果：{result.return_value if result.success else result.stderr}\n")
    
    # 测试3：通用任务（可能选择通用助手）
    print("\n测试3：通用编程任务")
    print("-" * 40)
    result = cognitive_agent.execute_sync("编写一个Python函数，判断一个数是否为素数")
    print(f"结果：{result.return_value if result.success else result.stderr}\n")
    
    # 打印执行历史
    print("\n执行历史：")
    print("-" * 40)
    for history in cognitive_agent.execution_history[-10:]:
        print(history)
